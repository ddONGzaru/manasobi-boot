...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    EXCEL_DOWNLOAD: function (caller, act, data) {
        ...
        window.location = CONTEXT_PATH + "/api/v1//mng/etc/sh04001200/download?" + params;
        return false;
    },
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                ...
                {key: 'corpCodeName', label: '회사코드', width: 70, align: 'center', editor: 'text'},
                {key: 'inoutDatetime', label: '출입일시', width: 130, align: 'center', editor: 'text'},
                {key: 'inoutGubunName', label: '출입구분', width: 70, align: 'center', editor: 'text'},
                {key: 'taskGubunName', label: '업무구분', width: 70, align: 'center', editor: 'text'},
                {key: 'authEmpNo', label: '카드(직원)번호', width: 100, align: 'center', editor: 'text'}
            ],
            body: {
                onClick: function () {
        ...
        axboot.buttonClick(this, "data-grid-view-01-btn", {
            ...
            "excel": function () {
                _this.excel("코너입퇴실시간정보-"+getFormattedDate(new Date())+".xls");
            }
        });
    },
    getData: function (_type) {
        ...
        if (_type == "modified" || _type == "deleted") {
            list = ax5.util.filter(_list, function () {
                return this.branchCode && this.cornerCode && this.inoutDatetime;
            });
        } else {
            list = _list;
        }
