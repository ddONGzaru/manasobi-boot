...
var ACTIONS = axboot.actionExtend(fnObj, {
    PAGE_SEARCH: function (caller, act, data) {
        axboot.ajax({
            type: "GET",
            url: "/api/v1//mng/error/sh01001180",
    ...
    EXCEL_DOWNLOAD: function (caller, act, data) {
        ...
        console.log(params);
        window.location = CONTEXT_PATH + "/api/v1//mng/error/sh01001180/download?" + params;
        return false;
    },
    ...
    ITEM_CLICK: function (caller, act, data) {
        ...
        fnObj.formView01.setFormData("errorMethodName", parent.COMMON_CODE["ERROR_METHOD"].map[data.errorMethod]);
        fnObj.formView01.setFormData("errorTypeName", parent.COMMON_CODE["SIMULTANEOUS_BUNDLE_READING_ERROR_GUBUN"].map[data.errorType]);
        fnObj.formView01.setFormData("totalClassifyCodeName", parent.COMMON_CODE["TOTAL_CLASSIFY_CODE"].map[data.totalClassifyCode]);
        /*axboot.ajax({
         type: "GET",
         url: "/api/v1/mng/error/sh01001120",
         data: {
        ...
        if (fnObj.formView01.validate()) {
            axDialog.confirm({
                ...
                if (this.key == "ok") {
                    ...
                    parentData.memoContent = parentData.memoContent || '';
                    parentData.calleeReqDate = parentData.calleeReqDate;

                   /* var inputData = {
                        handleDatetime: parentData.handleDate + " " + parentData.handleTime + ":00",
                        mngStndDate: parentData.mngStndDate,
                        mngStndCritic: parentData.mngStndCritic,
                        errorCount: parentData.errorCount,
                        ...
                        cornerName: parentData.cornerName,
                        handleEmpName: parentData.handleEmpName || '',
                        memoContent: parentData.memoContent || '',
                        calleeReqDate: parentData.calleeReqDate,
                        calleeSeqNo: parentData.calleeSeqNo,
                        errorMethod: parentData.errorMethod
                    };*/
                    axboot
                        .call({
                            type: "PUT",
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                ...
                {
                    key: 'modelCode', label: '기종', width: 100, align: 'left', formatter: function formatter() {
                    return parent.COMMON_CODE["TERMINAL_MODEL_CODE"].map[this.item.terminalNo + this.item.modelCode];
                }
                },
                {key: 'mngStndDate', label: '관리기준일', width: 80, align: 'center', editor: 'text'},
                {key: 'mngStndCritic', label: '관리기준 임계치', width: 80, align: 'center', editor: 'text'},
                {key: 'errorCount', label: '장애발생건수', width: 90, align: 'center', editor: 'text'},
                {key: 'calleeReqSeqNo', label: '출동일련번호', width: 100, align: 'center', editor: 'text'},
                {
                    key: 'errorMethod',
                    label: '장애발생매체',
                    width: 100,
                    ...
                    formatter: function formatter() {
                        return parent.COMMON_CODE["ERROR_METHOD"].map[this.value];
                    }
                },
                {
                    key: 'errorType',
                    ...
                    editor: 'text',
                    formatter: function formatter() {
                        return parent.COMMON_CODE["SIMULTANEOUS_BUNDLE_READING_ERROR_GUBUN"].map[this.value];
                    }
                },
                {
                ...
                {key: 'calleeReqDate', label: '출동요청일', width: 130, align: 'center', editor: 'text'},
                {key: 'memoContent', label: '메모사항', width: 150, align: 'left', editor: 'text'},
            ],
            body: {
                onClick: function () {
