...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    EXCEL_DOWNLOAD: function (caller, act, data) {
        ...
        console.log(params);
        window.location = CONTEXT_PATH + "/api/v1//mng/error/sh01001150/download?" + params;
        return false;
    },
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                ...
                {key: 'handleResult', label: '조치결과', width: 100, align: 'center', editor: 'text', formatter: function formatter() {
                    return parent.COMMON_CODE["HANDLE_RESULT"].map[this.value];
                }},
                {key: 'handleFailReason', label: '조치불가 사유', width: 100, align: 'left', editor: 'text', formatter: function formatter() {
                    return parent.COMMON_CODE["HANDLE_FAIL_REASON"].map[this.value];
                }},
                {key: 'asMethod', label: 'A/S 매체', width: 100, align: 'left', editor: 'center', formatter: function formatter() {
                    return parent.COMMON_CODE["AS_METHOD"].map[this.value];
                }},
                {key: 'handleDatetime', label: '조치일시', width: 130, align: 'center', editor: 'text'},
                {key: 'handleEmpName', label: '조치자명', width: 100, align: 'center', editor: 'text'},
                {key: 'handleEmpTelno', label: '조치자전화번호', width: 110, align: 'center', editor: 'text'},
                {key: 'handleContentCode', label: '조치내용코드', width: 100, align: 'left', editor: 'text'},
