...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    EXCEL_DOWNLOAD: function (caller, act, data) {
        ...
        console.log(params);
        window.location = CONTEXT_PATH + "/api/v1//mng/error/sh01001120/download?" + params;
        return false;
    },
...
fnObj.pageButtonView = axboot.viewExtend({
    initView: function () {
        axboot.buttonClick(this, "data-page-btn", {
            ...
            "excel": function () {
                fnObj.gridView01.excel("출동요청통보 이력-"+getFormattedDate(new Date())+".xls");
    ...
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                ...
                {key: 'calleeGubun', label: '출동구분', width: 80, align: 'center', editor: 'text', formatter: function formatter() {
                    return parent.COMMON_CODE["CALLEE_GUBUN"].map[this.value];
                }},
                // {key: 'branchCode', label: '지점코드', width: 100, align: 'left', editor: 'text'},
                // {key: 'cornerCode', label: '코너코드', width: 100, align: 'left', editor: 'text'},
                /*{key: 'terminalCorpCode', label: '기기사코드', width: 100, align: 'left', editor: 'text'},*/
                ...
                {key: 'calleeReqReasonCode', label: '출동요청사유', width: 100, align: 'left', editor: 'text', formatter: function formatter() {
                    return parent.COMMON_CODE["CALLEE_REQ_REASON_CODE"].map[this.value];
                }},
                {key: 'calleeEmpName', label: '출동요원명', width: 100, align: 'left', editor: 'text'},
                {key: 'calleeEmpTelno', label: '출동요원전화번호', width: 120, align: 'center', editor: 'text'},
               /* {key: 'terminalErrorCode1', label: '기기장애코드1', width: 100, align: 'left', editor: 'text'},
                {key: 'terminalErrorCode2', label: '기기장애코드2', width: 100, align: 'left', editor: 'text'},*/
                ...
                {key: 'calleeReqUnusl', label: '출동요청특이사항', width: 150, align: 'left', editor: 'text'},
                {key: 'crtNo', label: 'CRT-NO', width: 100, align: 'left', editor: 'text'},
                {key: 'customerWaitEnable', label: '고객대기여부', width: 100, align: 'center', editor: 'text', formatter: function formatter() {
                    return parent.COMMON_CODE["CUSTOMER_WAIT_ENABLE"].map[this.value];
                }},
                {key: 'partMngCalleeEnable', label: '부분관리출동여부', width: 120, align: 'center', editor: 'text', formatter: function formatter() {
                    return parent.COMMON_CODE["PART_MNG_CALLEE_ENABLE"].map[this.value];
                }}
            ],
            body: {
                onClick: function () {
